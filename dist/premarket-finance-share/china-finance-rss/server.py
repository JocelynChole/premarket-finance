...
Sources: Sina Finance (新浪财经), Eastmoney (东方财富), THS (同花顺), Xueqiu (雪球)
...
def handle_cls_telegraph():
    """Sina Finance 7x24 Telegraph (新浪财经 7x24 - 历史路由保留)

    注意：/cls/telegraph 路径保留 cls 前缀，但内部已切换到 handle_sina_kuaixun。
    本路由不再请求 cls.cn，统一从新浪财经抓取。
    """
    return handle_sina_kuaixun()
...
def handle_eastmoney_kuaixun():
    ...
    for item in result.get('LivesList', []):
        ...
        # 东方财富 API 的 url_w 字段可能返回搜索页链接，不是原文。
        # 原文固定格式：https://finance.eastmoney.com/a/{newsid}.html
        original_url = f'https://finance.eastmoney.com/a/{newsid}.html' if newsid else ''
        items.append({
            'title': item.get('title', ''),
            'link': original_url,
            'description': item.get('digest', ''),
            'pubDate': pubdate,
            'guid': f"eastmoney_{newsid}"
        })
...
def handle_ths_kuaixun():
    ...
    for item in data.get('data', {}).get('list', []):
        seq = item.get('seq', '')
        # 优先使用 API 返回的 url；降级时构造正确格式
        # 正确格式：https://news.10jqka.com.cn/{日期}/c{seq}.shtml
        api_url = item.get('url', '')
        if api_url and 'baidu.com' not in api_url and 'zs/search' not in api_url:
            link = api_url
        elif seq:
            ts = int(item.get('ctime', 0))
            date_str = datetime.fromtimestamp(ts, tz=timezone(timedelta(hours=8))).strftime('%Y%m%d')
            link = f'https://news.10jqka.com.cn/{date_str}/c{seq}.shtml'
        else:
            link = ''
        items.append({
            'title': item.get('title', ''),
...
ROUTES = {
    '/cls/telegraph': ('Sina Finance (新浪财经7x24)', handle_sina_kuaixun),  # 历史路由，内部已切到新浪财经
