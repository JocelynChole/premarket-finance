...
async function loadReport() {
  ...
  const userSendkey = localStorage.getItem('pms_pushplus_token') || '';
  const url = userSendkey
    ? `/api/report/today?pushplus_token=${encodeURIComponent(userSendkey)}`
    : '/api/report/today';
  try {
...
async function refreshReport() {
  ...
  try {
    ...
    await fetch('/api/refresh', {
      ...
      body: JSON.stringify({ pushplus_token: userSendkey })
    });
    await loadReport();
  } catch (e) {
