...
> 自动抓取新浪财经、东方财富、同花顺的预测类快讯 → 智能识别影响板块 → 结构化 Markdown 简报 → 一键推送个人微信。
...
2. 访问 [pushplus.plus](https://pushplus.plus/) 微信扫码登录，复制你的 `Pushplus Token`
3. 在订阅页填邮箱 + Pushplus Token + 关注的板块
...
| 订阅 | `/subscribe` | Pushplus 订阅表单 + 测试推送 |
| 档案 | `/history` | 历史简报时间线 |
...
### Q2: 微信没收到推送？
A: 三个排查点：
- 浏览器先点「发送测试消息」→ 验证 Pushplus Token 是否正确
- 登录 [pushplus.plus](https://pushplus.plus/) → 检查微信是否绑定
- 订阅时是否填了 Pushplus Token 且保存成功

### Q3: 推送内容很短 / 不完整？
A: Pushplus 免费版每天 200 条 + 单条 2000 字以内。盘前简报设计在 1-2 条额度内，超出会截断。可付费升级到 VIP。

### Q4: 抓取失败 / 报错？
A: 新浪财经、东方财富、同花顺均为公开接口，可能受反爬影响，可：
- 重试（点「立即刷新」）
...
想把项目**部署到公网、生成可分享的链接**？推荐用 **腾讯云轻量应用服务器**（新用户免费1个月）。

- 部署脚本：双击 `deploy_pmf.py` 一键部署
- 一键部署时间：约 5 分钟
- 拿到的链接形如 `http://你的公网IP:5000`，可分享给任何人直接点击


**简要流程**：
1. 注册腾讯云（微信扫码，无需信用卡）→ 实名认证 → 领取免费试用
2. 创建轻量应用服务器（Ubuntu 22.04，2核2GB）
3. 运行 `python deploy_pmf.py` 一键部署
4. 浏览器访问 `http://你的公网IP:5000`
5. 发给朋友 🎉
